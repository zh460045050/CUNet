...
class Solver(object):
		...
		if os.path.isfile(unet_path):
			...
			for epoch in range(self.num_epochs):
				...
				for i, (images, GTs, filename) in enumerate(self.train_loader):
					...
					loss_adr = torch.abs(loss_blood.clone() - loss_choroid.clone())
	...
	def validation(self, epoch):
		...
		for i, (images, GTs, filename)  in enumerate(self.valid_loader):
			...
			with torch.no_grad():
				SRs, fs, fd_c, fd_b  = self.unet(images)
	...
	def test(self):
		...
		for i, (images, GTs, filename)  in enumerate(self.valid_loader):
			...
			for cur_b in range(0, b):
				...
				###save features####
				save_features(fs[cur_b, :, :, :], feature_path, filename[cur_b], type='global')
				save_features(fd_c[cur_b, :, :, :], feature_path, filename[cur_b], type='choroid')
				save_features(fd_b[cur_b, :, :, :], feature_path, filename[cur_b], type='blood')
				...
				np.save(save_feature_path + "/" + filename[cur_b] + "_blood", fd_b[cur_b, :, :, :])
				np.save(save_feature_path + "/" + filename[cur_b] + "_choroid", fd_c[cur_b, :, :, :])
				np.save(save_feature_path + "/" + filename[cur_b] + "_global", fs[cur_b, :, :, :])
